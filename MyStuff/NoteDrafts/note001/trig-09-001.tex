...
The efficiency of the trigger drops in the geometrical regions
 of separation between two adjacent sectors or wheels.

In the top part (Figure~\ref{fig:eff_eta_phi_top_08})
along the $\phi$ coordinate there are 7 regions
corresponding to the sectors 1 to 7 going from the left to the right.

In the bottom part (Figure~\ref{fig:eff_eta_phi_bot_08})
the variable on the x-axis is $360 - \phi$.
There are 7 regions corresponding to sectors 12 to 7 
...
%The pointing requirement on the tracks implies
%that the muons passing through sectors 1 and 7 are 

%Chambers of sectors 1 and 7 are disposed with 
%the RPC plates perpendicular to the ground.
...
For the considered run, sectors 1 and 7 of the DTs were off from the
trigger, so there is almost no statistics at all for those towers.

The events in which the trigger is efficient in sector 1-7 are due
to the  $30^\circ$   $\Delta \phi$ interval chosen for the matching 
between the reconstructed track and the trigger candidates.

%%%%

%The efficiency is not uniform over all
%the detector, depending on local condition 
%of that regin detector the sectors 
%where the layers were off. 

%In particular the efficiency

%turned off due   


%This is explainable with the
%experim
...
%In the geometrical region between two sectors however the number of 
%available patterns is limited and the DT and RPC trigger are correlated.

%The algorythms for DT and RPC triggers have different logics,
%and independent hardware. The problem is that.
...
%The behavior of efficiency at low \pt can be explained
%considernig that the tracks with lower transverse momentum
%bend more in the magnetic field. The cones are 
%optimized for tracks bending in the magnetic field but 
%still pointing to the vertex, so for cosmics tracks 

The efficiency reachs a plateau after about $40 GeV/c$.

After the volume cuts, the efficiency for  \pt $ > 40 $ is 
...
\begin{equation} 
\\$\epsilon_{Plateau08} = 88.98 \pm 0.33 % $
\end{equation} 
...
The plots have the same conventions as in Figures
 ~\ref{fig:eff_eta_phi_bot_08} and ~\ref{fig:eff_eta_phi_bot_09}.

It is possible to see also the same structures as for CRAFT08, but 
the efficiency generally higher.% A reason for this will be given 
%in section

For the CRAFT09 run considered, sectors 1 and 7 of the DTs were off 
from the trigger except for wheel0 and wheel+1  
...
The results from sections ~\ref{eff_08}-~\ref{eff_09} show that there is a differece of about
$9 \% $ between $epsilon_{Plateau08}$ and $epsilon_{Plateau09}$ .
